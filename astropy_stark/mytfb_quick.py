...
def pytfb_sub(taus, embh, emdot, wavang, deginc, t0vin=-1, t0iin=-1, alpha_visc=-0.75, hxsch=3.0, alpha_irad=-0.75,
              eta=0.1, rlosch=3.0, norm=1, quick=1, xstop=15, udlnr=0.01, thcent=1.0, thfwhm=0.2, oldsmooth=0,
    ...
    if (wavang < 0.0):
        ...
        if (quick == 1):
            for i in range(nrad - 1):
                ...
                weight = rir0b[i] / ttot3now * x2 / wavang2 / (np.cosh(
                    x) - 1) * radlo * drad * azwidth  # rir0b/ttot3 * x2/wavang2/(np.cosh(x) - 1) * radlo*drad*azwidth      #
            ...
            for i in range(nrad - 1):
                ...
                daz = np.sqrt(raz * raz + hx2)
                az = np.random.uniform(low=azgrid[:-1], high=azgrid[1:],
                                       size=nazsub1)  # np.random.uniform(azgrid[:-1],azgrid[1:],1)
                caz = np.cos(az)
                tdl = hxci - raz * caz * sini + daz
                tv4 = t0v4 * (
                    raz) ** av4  # modification for inner radius to the right (negligible)* (1 - np.sqrt(rlold/raz)) / (1 - np.sqrt(rlold/1))
                ti4 = t0i4 * (raz) ** ai4
                rir0b = ti4 / t0i4  # this is just (r/r0)^alpha_irad
                ttot4 = tv4 + ti4
                ttot2 = np.sqrt(ttot4)
